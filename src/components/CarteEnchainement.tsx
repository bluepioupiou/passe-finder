  ...
  return (
    <Link className="enchainement-carte" href={`/enchainements/${enchainement.id}`}>
      ...
      <p className="enchainement-meta texte-attenue">
        ...
        {/* JUSTE LA NOTE, a la suite de la date et du nombre de passes : dans
            une grille, ce qu'on veut savoir c'est « celui-ci en a une ». Le
            morceau lui-meme se lit sur la fiche — l'ecrire ici (« Ecouter sur
            Spotify »…) allongeait la ligne sans rien apprendre d'utile au
            balayage.
            ...
            deja nomme (UX-DR17). */}
        {musique ? (
          <span className="enchainement-musique">
            {' · '}
            <IconeNote taille={14} />
            <span className="enchainement-musique__intitule">Avec musique</span>
          </span>
        ) : null}
      </p>
