...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`danses_nom_idx\` ON \`danses\` (\`nom\`);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`danses_id\` integer REFERENCES danses(id);`)
