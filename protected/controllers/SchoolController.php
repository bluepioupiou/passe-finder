...
class SchoolController extends Controller
{
	...
	public function accessRules()
	{
		...
		if (Yii::app()->request->getParam('id')){
			$school = School::model()->findByPk(Yii::app()->request->getParam('id'));
			if(Yii::app()->user->getId() === $school->userManager_id) {
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['School']))
		{
			...
			$model->userManager_id=Yii::app()->user->id;
			...
			if($model->save()){
				...
				$email->subject = 'Nouvelle Ecole créé par '.Yii::app()->user->username;
				$email->message = "<a href='http://www.passe-finder.fr/index.php/school/".$model->id."'>Lien vers la nouvelle école</a>";
				$email->send();
	...
	public function actionIndex()
	{
		$dataProvider=new CActiveDataProvider('School', array(
			'criteria'=>array(
				'condition'=>'pending=0 or userManager_id='.Yii::app()->user->id			
			)
		));
